...
template<int numBits> struct Bitset
{
	...
	bool Get(int idx) const { return !!(bits[idx >> 6] & (1ul << idx & 63)); }
	...
	// std::string toString() {
	//  std::string str; str.resize(numBits); for (int i = 0, currBit = 0; i < size; ++i) for (int j = 0; j < 64 && currBit < numBits; ++j, ++currBit) str[currBit] = (bits[i] & (1ul << j) ? '1' : '0');
	// 	return str; }
};
